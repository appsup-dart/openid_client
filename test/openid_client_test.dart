...
import 'dart:convert';
import 'dart:async';
import 'package:openid_client/src/cache.dart';

File _file(String path) {
  return new File("${Directory.current.path.endsWith("test") ? "" : "test/"}$path");
}

Future<dynamic> _readJson(String path) async => JSON.decode(await _file(path).readAsString());
