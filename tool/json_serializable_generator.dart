...
String _fieldToAnnotatedMapValue(String defaultValue, FieldElement field) {
  ...
  return _camelToUnderscore(defaultValue);
}
...
String _writeAccessToVar(String varExpression, DartType searchType,
  ...
  if (_isInt(searchType)) {
    return "$varExpression == null ? null : $varExpression is String ? "
        "int.parse($varExpression) : $varExpression";
  }

  if (_isUri(searchType)) {
    return "$varExpression == null ? null : Uri.parse($varExpression)";
  }

  if (_isDartDateTime(searchType)) {
    // TODO: this does not take into account that dart:core could be
    // imported with another name
    return "$varExpression == null ? null : $varExpression is num ? "
        "new DateTime.fromMillisecondsSinceEpoch(($varExpression*1000).toInt()) : "
        "DateTime.parse($varExpression)";
  }
