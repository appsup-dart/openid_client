  ...
  Set<ConstraintViolation> validatePayload(
    ...
    if (claimSetValidationContext is IdTokenValidationContext) {
      if (claimSetValidationContext.validateIssuerAndAudience) {
        ...
        if (!openIdClaimsSet.audience
            .contains(claimSetValidationContext.clientId)) {
          set.add(
              new ConstraintViolation('Audiences does not contain clientId.'));
        }
        if (openIdClaimsSet.audience.length > 1 &&
            openIdClaimsSet.authorizedParty == null) {
          set.add(
              new ConstraintViolation('No authorized party claim present.'));
        }
        if (openIdClaimsSet.authorizedParty != null &&
            openIdClaimsSet.authorizedParty !=
                claimSetValidationContext.clientId) {
      ...
      if (claimSetValidationContext.nonce != null &&
          claimSetValidationContext.nonce != openIdClaimsSet.nonce) {
